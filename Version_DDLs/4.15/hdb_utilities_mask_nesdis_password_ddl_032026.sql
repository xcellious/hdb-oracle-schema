---------------------------------------------------------------------------
-- This script add new function mask_nesdis_password to HDB_UTILITIES package. 
...
FUNCTION mask_nesdis_password (arg IN VARCHAR2) 
  RETURN VARCHAR2 
IS
    new_arg VARCHAR2(400);
BEGIN
    new_arg := REGEXP_REPLACE(
                  arg,
                  '(password=)([^,]+)',
                  '\1' || 'masked-nesd-ISX9-99-password'
               );

    RETURN new_arg;
END;   /* end of Function mask_nesdis_password  */
...
--compile
ALTER PACKAGE HDB_UTILITIES COMPILE;
SHOW ERRORS PACKAGE HDB_UTILITIES;

ALTER PACKAGE HDB_UTILITIES COMPILE BODY;
SHOW ERRORS PACKAGE BODY HDB_UTILITIES;

-- Stop script if any error occurs
WHENEVER SQLERROR EXIT FAILURE;

-- Only runs if compile succeeded
INSERT INTO TSDB_PROPERTY (PROP_NAME, PROP_VALUE)
VALUES (
    '4.15.8',
    'New Function mask_nesdis_password has been added to HDB_UTILITIES package'
);
